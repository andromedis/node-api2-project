...
describe('server.js', () => {
  describe('1 [GET] /api/posts', () => {
    it('can get the correct number of posts', async () => {
      let res: Response = await request(server).get('/api/posts')
  ...
  describe('2 [GET] /api/posts/:id', () => {
    it('can get all the correct posts by id', async () => {
      await db('posts').insert(post1)
      await db('posts').insert(post2)
      let res: Response = await request(server).get('/api/posts/1')
    ...
    it('reponds with a 404 if the post is not found', async () => {
      let res: Response = await request(server).get('/api/posts/111')
  ...
  describe('3 [POST] /api/posts', () => {
    it('responds with a 201', async () => {
      const res: Response = await request(server).post('/api/posts').send(post1)
      expect(res.status).toBe(201)
    }, 500)
    it('responds with a new post', async () => {
      let res: Response = await request(server).post('/api/posts').send(post1)
      expect(res.body).toHaveProperty('id')
      expect(res.body).toMatchObject(post1)
    ...
    it('on missing title or contents responds with a 400', async () => {
      let res: Response = await request(server).post('/api/posts').send({ title: 'foo' })
  ...
  describe('4 [PUT] /api/posts/:id', () => {
    it('responds with updated post', async () => {
      const [ id ]: [id: number] = await db('posts').insert(post1)
      const updates: BasePost = { title: 'xxx', contents: 'yyy' }
      const res: Response = await request(server).put(`/api/posts/${id}`).send(updates)
      expect(res.body).toMatchObject({ id, ...updates })
    }, 500)
    it('saves the updated post to the db', async () => {
      const [ id ]: [id: number] = await db('posts').insert(post2)
      const updates: BasePost = { title: 'aaa', contents: 'bbb' }
      await request(server).put(`/api/posts/${id}`).send(updates)
      let post: Post | undefined = await db('posts').where({ id }).first()
      expect(post).toMatchObject({ id, ...updates })
    }, 500)
    it('responds with the correct message & status code on bad id', async () => {
      const updates: BasePost = { title: 'xxx', contents: 'yyy' }
      const res: Response = await request(server).put('/api/posts/foobar').send(updates)
      expect(res.status).toBe(404)
      expect(res.body.message).toMatch(/does not exist/i)
    }, 500)
    it('responds with the correct message & status code on validation problem', async () => {
      const [ id ]: [id: number] = await db('posts').insert(post2)
      let updates: { [key: string]: any } = { title: 'xxx' }
      let res: Response = await request(server).put(`/api/posts/${id}`).send(updates)
      expect(res.status).toBe(400)
  ...
  describe('5 [DELETE] /api/posts/:id', () => {
    it('reponds with a 404 if the post is not found', async () => {
      let res: Response = await request(server).delete('/api/posts/111')
      expect(res.status).toBe(404)
      expect(res.body.message).toMatch(/does not exist/i)
    }, 500)
    it('reponds with the complete deleted post', async () => {
      await db('posts').insert(post1)
      let res: Response = await request(server).delete('/api/posts/1')
      expect(res.body).toHaveProperty('id')
      expect(res.body).toMatchObject(post1)
    }, 500)
  ...
  describe('6 [GET] /api/posts/:id/comments', () => {
    it('reponds with a 404 if the post is not found', async () => {
      let res: Response = await request(server).get('/api/posts/66/comments')
      expect(res.status).toBe(404)
      expect(res.body.message).toMatch(/does not exist/i)
    }, 500)
    it('can get all the comments associated to the posts with given id', async () => {
      ...
      let res: Response = await request(server).get('/api/posts/1/comments')
      expect(res.body).toHaveLength(2)
